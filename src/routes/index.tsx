    ...
    <header className="sticky top-0 z-50 glass-card border-b border-outline-variant/40">
      <div className="mx-auto flex max-w-[1200px] items-center justify-between px-6 py-3">
        ...
        <nav className="hidden items-center gap-8 md:flex">
          {links.map((l) => (
            <a
              key={l.href}
              href={l.href}
              className="font-mono text-[13px] uppercase tracking-[0.08em] text-on-surface-variant hover:text-primary transition-colors"
            >
              {l.label}
            </a>
          ))}
        ...
        <button
          ...
          className="md:hidden rounded-lg p-2 text-on-surface"
          aria-label="menu"
        >
        ...
        <div className="md:hidden border-t border-outline-variant/40 bg-surface-container-lowest px-6 py-4 space-y-3">
          {links.map((l) => (
            <a
              ...
              className="block font-mono text-[13px] uppercase tracking-[0.08em] text-on-surface-variant"
            >
              {l.label}
            </a>
          ))}
